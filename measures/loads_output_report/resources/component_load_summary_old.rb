class ComponentLoadSummaryOld
  attr_accessor :sql_file

  BASE_QUERY = "SELECT Value FROM TabularDataWithStrings"
  LOAD_ROWS = ['People', 'Lights', 'Equipment', 'Refrigeration', 'Water Use Equipment', 'HVAC Equipment Losses', 'Power Generation Equipment',
  ...
  LOAD_COLUMNS = ['Sensible - Instant', 'Sensible - Delayed', 'Sensible - Return Air', 'Latent', 'Total']
  PEAK_ROWS =     ["Time of Peak Load", "Outside  Dry Bulb Temperature", "Outside  Wet Bulb Temperature", "Outside Humidity Ratio at Peak",
  ...
  def get_cooling_load_summary(report_for_string)
    ...
    LOAD_ROWS.each do |row_name|
      row_loads = {}

      LOAD_COLUMNS.each do |column_name|
