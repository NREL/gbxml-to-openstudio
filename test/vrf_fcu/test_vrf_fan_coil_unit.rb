...
class TestVRFFanCoilUnit < MiniTest::Test
  ...
  def before_setup
    self.gbxml_path = TestConfig::GBXML_FILES + '/VRFAllVariations.xml'
  ...
  def test_build
    ...
    assert(fcu.additionalProperties.getFeatureAsString('id').get == 'aim0832')
    assert(fcu.additionalProperties.getFeatureAsString('CADObjectId').get == '280066-1')
  end

  def test_create_osw
    osw = create_gbxml_test_osw
    osw = add_gbxml_test_measure_steps(osw, 'VRFAllVariations.xml')
    osw_in_path = TestConfig::TEST_OUTPUT_PATH + '/vrf_fcu/in.osw'
  ...
  def test_simulation
    ...
    osw_out_path = TestConfig::TEST_OUTPUT_PATH + '/vrf_fcu/out.osw'
