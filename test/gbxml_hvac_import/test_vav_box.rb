...
class TestVAVBox < MiniTest::Test
  ...
  def before_setup
    self.gbxml_path = Config::GBXML_FILES + '/VAVBoxAllVariations.xml'
  ...
  def test_build
    ...
    assert(vav_box_elec.additionalProperties.getFeatureAsString('id').get == 'aim0936')
    assert(vav_box_elec.additionalProperties.getFeatureAsString('CADObjectId').get == '280066-1')
  end

  def create_osw
    osw = create_test_sizing_osw
    osw = adjust_gbxml_paths(osw, 'VAVBoxAllVariations.xml')
    osw_in_path = Config::TEST_OUTPUT_PATH + '/vav_box/in.osw'
  ...
  def test_simulation
    ...
    osw_out_path = Config::TEST_OUTPUT_PATH + '/vav_box/out.osw'
