...
class TestCharacterSets < MiniTest::Test
  def test_create_osws
    ...
    osw_in_path = Config::TEST_OUTPUT_PATH + '/character_sets/in.osw'
    ...
    osw_rename_path = Config::TEST_OUTPUT_PATH + '/character_sets/模型/in.osw'
    File.rename(osw_in_path, osw_rename_path)

    osw = create_gbxml_test_osw
    osw = add_gbxml_test_measure_steps(osw, 'chinese_gbxml.xml')
    osw_in_path = Config::TEST_OUTPUT_PATH + '/character_sets/in_gbxml.osw'
    osw.saveAs(osw_in_path)

    osw = create_gbxml_test_osw
    osw = add_gbxml_test_measure_steps(osw, '模型.xml')
    osw_in_path = Config::TEST_OUTPUT_PATH + '/character_sets/measure_arg.osw'
    osw.saveAs(osw_in_path)

    osw = create_gbxml_test_osw
    osw = add_gbxml_test_measure_steps(osw, 'CAVBoxAllVariations.xml', weather_file_name: '듥듦.epw')
    osw_in_path = Config::TEST_OUTPUT_PATH + '/character_sets/weather.osw'
    osw.saveAs(osw_in_path)

    osw = create_gbxml_test_osw
    osw = add_gbxml_test_measure_steps(osw, 'CAVBoxAllVariations.xml')
    osw_in_path = Config::TEST_OUTPUT_PATH + '/character_sets/chinese_characters.osw'
    osw.saveAs(osw_in_path)
    # add .force_encoding('UTF-8') to osw_in_path
    # rename the .osw once written for now; update once tests are guaranteed to use >= OS v2.7.1
    osw_rename_path = Config::TEST_OUTPUT_PATH + '/character_sets/模型.osw'
    File.rename(osw_in_path, osw_rename_path)
  end
  ...
  def test_in_measure_arg
    ...
    osw_out_path = Config::TEST_OUTPUT_PATH + '/character_sets/out.osw'
  ...
  def test_in_gbxml
    gbxml_path = Config::GBXML_FILES + '/chinese_gbxml.xml'
  ...
  def test_in_osw_path
    ...
    osw_in_path = Config::TEST_OUTPUT_PATH + '/character_sets/듥듦/in.osw'
    cmd = "\"#{Config::CLI_PATH}\" run -w \"#{osw_in_path}\""
    assert(run_command(cmd))

    osw_out_path = Config::TEST_OUTPUT_PATH + '/character_sets/듥듦/out.osw'
    osw_out = JSON.parse(File.read(osw_out_path))
  ...
  # set osw_path to find location of osw to run
    osw_in_path = Config::TEST_OUTPUT_PATH + '/character_sets/模型.osw'
    cmd = "\"#{Config::CLI_PATH}\" run -w \"#{osw_in_path}\""
    assert(run_command(cmd))
