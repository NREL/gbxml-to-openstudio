...
class TestCAVBox < MiniTest::Test
  ...
  def before_setup
    self.gbxml_path = Config::GBXML_FILES + '/CAVBoxAllVariations.xml'
  ...
  def test_build
    ...
    cav_box_elec = self.model_manager.zone_hvac_equipments.values[0].air_terminal
    cav_box_furn = self.model_manager.zone_hvac_equipments.values[1].air_terminal
    cav_box_hw = self.model_manager.zone_hvac_equipments.values[2].air_terminal
    cav_box_no_rh = self.model_manager.zone_hvac_equipments.values[3].air_terminal

    assert(cav_box_elec.reheatCoil.to_CoilHeatingElectric.is_initialized)
    assert(cav_box_elec.is_a?(OpenStudio::Model::AirTerminalSingleDuctConstantVolumeReheat))

    assert(cav_box_furn.reheatCoil.to_CoilHeatingGas.is_initialized)
    assert(cav_box_hw.reheatCoil.to_CoilHeatingWater.is_initialized)
    assert(cav_box_no_rh.is_a?(OpenStudio::Model::AirTerminalSingleDuctUncontrolled))

    # only need to test one object for this mapping
    assert(cav_box_elec.name.get == 'CAV Box Electric')
    assert(cav_box_elec.additionalProperties.getFeatureAsString('id').get == 'aim0828')
  ...
  def test_simulation
    ...
    osw_out_path = Config::TEST_OUTPUT_PATH + '/cav_box/out.osw'
